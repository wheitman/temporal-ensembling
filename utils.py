...
def prepare_mnist():
    ...
    test_dataset = datasets.MNIST(
        root="../data", train=False, transform=tf.Compose([tf.ToTensor(), normalize])
    )
...
def save_exp(time, losses, sup_losses, unsup_losses, accs, accs_best, idxs, **kwargs):

    def save_txt(fname, accs, **kwargs):
        with open(fname, "w") as fp:
            fp.write("GLOB VARS\n")
            fp.write("n_exp        = {}\n".format(kwargs["n_seed_restarts"]))
            fp.write("k            = {}\n".format(kwargs["num_labels"]))
            fp.write("MODEL VARS\n")
            fp.write("drop         = {}\n".format(kwargs["drop"]))
            fp.write("std          = {}\n".format(kwargs["std"]))
            fp.write("fm1          = {}\n".format(kwargs["fm1"]))
